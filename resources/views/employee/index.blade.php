                ...
                <div class="row">
                    <div class="col-md-3">
                        <div class="form-group">
                            <label for="search_code">{{ __('employee.employee_code') }}</label>
                    ...
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="search_phone" name="search_phone" value="{{ request('search_phone') }}">
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="form-group">
                            <label for="search_email">{{ __('employee.email') }}</label>
                            <input type="text" class="form-control" id="search_email" name="search_email" value="{{ request('search_email') }}">
                        </div>
                    </div>
                </div>
                <div class="row mt-2">
        ...
        <div class="card-body">
            ...
            <table class="table nowrap border display" id="mytable" style="width:100%">
                <thead>
                    <tr>
                        <th>Action</th>
                        <th>{{ __('employee.employee_code') }}</th>
                ...
                <tbody>
                    @foreach($table as $employee)
                        <tr>
                            <td>
                                <a href="{{ route('employee.edit', $employee->employee_code) }}" class="btn btn-success btn-xs"><i class="fa fa-edit"></i></a>
                                <form action="{{ route('employee.destroy', $employee->employee_code) }}" method="POST" style="display:inline-block;">
                                    @csrf
                                    @method('DELETE')
